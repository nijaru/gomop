...
// PackageByName maps a package name (e.g., "json") to its full paths (e.g., ["encoding/json"]).
var PackageByName = make(map[string][]string)
...
func init() {
	...
	// Pre-calculate reverse index
	for path := range PackageSymbols {
		name := GetAssumedName(path)
		PackageByName[name] = append(PackageByName[name], path)
	}
}

// GetAssumedName returns the base name of a package path.
func GetAssumedName(path string) string {
	if i := strings.LastIndex(path, "/"); i != -1 {
		base := path[i+1:]
		if strings.HasPrefix(base, "v") && len(base) > 1 {
			// Handle versioned paths like /v2, /v8
			isVersion := true
			...
			if isVersion {
				// Re-split to get the actual package name
				sub := path[:i]
				if j := strings.LastIndex(sub, "/"); j != -1 {
					return sub[j+1:]
				}
				return sub
			}
